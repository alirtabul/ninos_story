...
When looking at the analysis without taking the `Budget` in account, it seems that the most positively impacting factors are the genres `Adventure`and `Action`, and the movie characteristics `English language`, `Movie runtime` and `Movie_release_year`. On the other side, the negatively impacting factors are related to the movie genre. It appears that the actors characteristics don't really impact much the movie success compare to other factors. Interestingly  `Crime` is a genre that is highly variable on the impact on the `Box office revenue`. Also when looking at the 
$R^2$ 
value, we can see that the value is equal to 0.21 which is relatively decent for real data. It's not particularly high but real data is usually hard to predict (lot of factors, cofactors,...). A quick reminder that the $R^2$ quantifies the variance of the data that can be explained.
